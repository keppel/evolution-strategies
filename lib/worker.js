...
module.exports = function(opts = {}) {
  ...
  socket.on('initialize', ({ hyperparameters, blocks, parameters }) => {
  ...
  socket.on('request-parameters', () => {
    socket.emit('parameters', { parameters: headParameters })
  })
...
function noise(sharedSeed, uniqueSeed, numParams, useCache) {
  ...
  const cacheSize = 1e6
  if (useCache && noiseCache.length === 0) {
    debug('generating noise cache')
    let rng = seedrandom(sharedSeed)
    for (let i = 0; i < cacheSize; i++) {
      noiseCache.push(distribution.ppf(rng.quick()))
    }
  ...
  return {
    sample(index) {
      let noiseVector = []
      if (useCache) {
        noiseVector = noiseCache.slice(index, index + numParams)
      } else {
