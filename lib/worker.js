...
module.exports = function (opts = {}) {
  ...
  let { master, fitness, initialParameters, syncEpisodes = true } = opts
  ...
  let sigma = 0.1
  let alpha = 0.01
  let optimizer = opts.optimizer === 'sgd' ? sgd(alpha) : adam(alpha)
...
function _applyUpdate (sigma, alpha, optimizer, blockSize, params, updateVector) {
  ...
  params.forEach((p, k) => {
    ...
    params[k] += alpha * optimizer(params[k], updateVector[k], k) / (blockSize * sigma)
    updateVector[k] = 0
  })
}
